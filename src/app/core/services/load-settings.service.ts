...
constructor(private http: HttpClient) { }
  public loadSettings(): Promise<any> {
      return new Promise<any>((resolve, reject) => {
        ...
        this.http.get('/assets/appsettings.json', { headers: headers, responseType: 'json'}).toPromise().then((data: any) => {
          ...
          this.handShake().then((res: string) => {
            GlobalFramework.AppSettings.sessionId = res;
  ...
  handShake(): Promise<string> {
      let url = `${GlobalFramework.AppSettings.hermesAPIUrl_Auth}HandShake`;
      let header = new HttpHeaders({ 'Content-Type': 'application/json'});
      return this.http.get<string>(url, {headers: header}).toPromise();
